...
export default function SettingsScreen() {
  ...
  useEffect(() => {
    if (countdown <= 0 && resultRef.current && !loading && !sessionExpiredRef.current) doRefresh();
  }, [countdown]);
  ...
  async function doRefresh() {
    ...
    if (!prev) return;
    if (Date.now() - sessionStartRef.current > SESSION_TIMEOUT_MS) { setSessionExpired(true); return; }
    try {
      const dataset      = refreshDataset(prev.dataset, Number(f.balanceRupees) * 100);
      const plaintext    = serialise(dataset);
      const signature    = await sign(plaintext);
      const finalPayload = assembleFinal(plaintext, signature);
      setResult({ dataset, plaintext, signature, finalPayload });
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
        ...
        <SVPForm values={form} onChange={setForm} onGenerate={handleGenerate} loading={loading} />
        <QRDisplay
          result={result}
          countdown={countdown}
          refreshSecs={Math.max(5, Number(form.refreshSecs) || 30)}
